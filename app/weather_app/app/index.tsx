...
export default function HomeScreen() {
    ...
    return (
        <ScrollView>
            <CitySearch onCitySelect={async (city) => {
            ...
            {weather && (
                <View>
                    ...
                    <Text>Ощущается как: {weather.current.apparent_temperature}°</Text>
                    <Text>Ветер: {weather.current.wind_speed_10m}</Text>
                    <Text>Влажность: {weather.current.relative_humidity_2m}%</Text>
                    <Text>Видимость: {weather.current.visibility}</Text>
                </View>
            )}
